...
@dataclass
class Idea3TrainCollator:
    data_args: DataArguments
    tokenizer: PreTrainedTokenizer

    def __call__(self, features: List[Tuple[str, List[str], float]]):
        """
        Collate function for training.
        :param features: list of (query, passages) tuples
        :return: tokenized query_ids, passage_ids
        ...
        for f in features:
            all_passages.extend(f[1])
        all_only_queries = [f[2] for f in features]
        all_inst_flags = [f[3] for f in features]
        ...
        all_inst_flags = torch.tensor(all_inst_flags)
        
        return q_collated, d_collated, only_q_collated, all_inst_flags
